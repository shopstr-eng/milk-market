...
**sf-active also kills the platform TopNav:** globals.css has `body.sf-active [data-main-nav] { display:none !important; }`. So the body class must only be set when seller chrome will actually render (storefront config loaded AND Pro-entitled), or on custom domain/self-host where \_app suppresses the TopNav anyway. Setting it eagerly for everyone left no-stall/non-Pro sellers' listing & cart pages with NO navbar at all.
