...
An auto-merged upstream feature can also reference a **local binding that downstream renamed**, which is invisible in a diff review but breaks compilation. Concrete case: upstream's saved-address feature (`applySavedAddress`) calls `setValue(...)`, but downstream aliases react-hook-form's setter as `setValue: formSetValue` in `cart-invoice-card.tsx` — the merged call referenced the non-existent `setValue` and only `tsc --noEmit` surfaced it. **Always run a full typecheck after a squash-merge/port, not just an eyeball diff**; the dev server's `next build` also catches it but tsc is faster.
