...
**How to apply:**
- Wire contracts were renamed with dual-compat, never hard-cut: writes publish new (`SelfSown`/`selfsown`/`self-sown-zapsnag` tags, `ss_donation`, `self-sown:nip46:v1`, `com.selfsown` UCP), reads accept BOTH old and new (fetch filters dual-match, donation dual-reads, NIP-46 legacy-context decrypt fallback). Replacement writers (republish helpers, MCP update) must route copied tags through `normalizeMarketplaceDiscoveryTag` so legacy tags are upgraded on any edit.
...
- Still NOT renamed by deliberate decision: `milkmarket_*` Stripe lookup keys (Stripe Prices are immutable — dashboard rename must precede dropping LEGACY_* constants), `milk-market.replit.app` CNAME fallback, `milkmarket:` deeplink scheme + `milkmarket` NIP-05 alias, `FREEMILK` promo tag, Stripe `mm_*` metadata keys, external account URLs.
