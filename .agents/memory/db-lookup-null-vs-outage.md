...
**Rule:** read accessors feeding payment/webhook flows must rethrow on DB error (callers that want null-on-error add their own `.catch`, as webhook.ts's handleInvoicePaid already does). The handler then treats null as "genuinely missing" (log a loud greppable marker, e.g. ORPHANED_SUBSCRIPTION_PAYMENT, return 200) and lets throws hit the outer catch (500 + `releaseStripeEvent` so Stripe's retry isn't deduped).
