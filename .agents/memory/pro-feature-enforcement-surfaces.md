...
**Client-UI gating of a Pro section must cover EVERY render branch, not just the list entry buttons.** A settings page that shows the UpgradeBanner often renders distinct branches: the list view, an already-open editor (`showEditor`/`editingFlow`), and modals (contact picker). Gating only the list's New/Edit/Delete buttons leaves the editor/modal actions enabled if the seller becomes lapsed WHILE that branch is open (or any other path sets the open-state without going through the disabled entry button). Wrap each branch's interactive region in `<fieldset disabled={!membership.isPro} className="m-0 border-0 p-0 disabled:opacity-60">` (add `space-y-4`/`flex` to the fieldset when it replaces a spacing/layout wrapper, since fieldset defaults to `display:block`). Keep only navigation/exit controls (Back, Cancel) OUTSIDE the fieldset so a lapsed seller mid-edit isn't trapped. `<fieldset disabled>` natively disables nested native buttons, `<select>`, inputs, AND HeroUI Button/Checkbox (they render real form controls). Gate on `membership.isPro` (from `useProMembership()`) — true only for trialing/active/grace — so BOTH free and read-only/lapsed sellers are blocked. **Why:** server endpoints already fail-closed via `requireProEntitlement`; this is UX/defense-in-depth, but "block all action buttons" means all reachable states, which an architect review will hold you to.

**How to apply:** when asked to gray-out/block a Pro section's actions, grep the page for every `return (` / modal branch, not just the main list; wrap each interactive region's mutating controls and leave exit controls enabled. Single buttons can use HeroUI `isDisabled={!membership.isPro}` instead of a fieldset.

**A disabled submit button does NOT block a text input's Enter-key submit.** When a Pro action is a modal/form whose input has an `onKeyDown` "Enter → submit" handler, disabling the button leaves a keyboard bypass (worse during the brief `useProMembership().loading` window, when the not-Pro branch may still render the input). Add a guard at the TOP of the submit handler itself (`if (proLoading || !isPro) return;`) in addition to `isDisabled` on the button and swapping the form for `<UpgradeBanner>`. **Why:** an architect review caught this exact hole in the import-design modal — the disabled "Build my design" button was bypassable via Enter while membership resolved.
