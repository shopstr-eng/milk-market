    ...
    >
      <div className="mx-auto max-w-4xl px-4 pt-10 pb-8 sm:px-6 lg:px-8">
                  ...
                  orderData.cartItems?.some((i) => i.pickupLocation) && (
                    <div>
                      ...
                      {orderData.cartItems
                        ...
                        .map((item, idx) => (
                          <p key={idx} style={{ color: colors.text }}>
                            <span style={{ color: colors.text + "99" }}>
                              {item.title}:
                            </span>{" "}
                            {item.pickupLocation}
        ...
        {sellerProducts.length > 0 && (
          <div className="mt-10">
            <h2
              className="font-heading mb-6 text-center text-2xl font-bold"
              style={{ color: colors.text }}
            >
              More From {shopName}
            </h2>
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {sellerProducts.map((product) => (
                <div
                  key={product.id}
                  className="overflow-hidden rounded-lg border-2 transition-transform hover:-translate-y-1"
