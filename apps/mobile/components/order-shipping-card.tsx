...
import { OrderReturnLabelCard } from "@/components/order-return-label-card";
...
export function OrderShippingCard(props: OrderShippingCardProps) {
  ...
  const canReturn =
    props.order.status === "shipped" || props.order.status === "completed";
    ...
    <>
        ...
        >
          {shipping.labels.map((label) => (
            ...
            >
              ...
              <DetailRow
                label="Tracking"
                value={label.trackingCode || "Pending"}
              />
              <ActionButton
      ...
      {canReturn ? (
        <OrderReturnLabelCard
          key={`${props.session.pubkey}:${props.order.orderId}`}
          session={props.session}
          order={props.order}
          from={shipping.shippingAddress}
