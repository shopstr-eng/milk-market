...
- **Magic-Link Auth**: Two passwordless email-link flows backed by `magic_link_tokens` (single-use, 1h TTL) and `magic_link_sessions` (bearer cookie `mm_session`, `HttpOnly; SameSite=Strict; Secure` in prod). Sign-in flow (`/api/auth/request-email-link` → `/api/auth/verify-email-link`, page `/auth/sign-in-link`, entry CTA "Email me a sign-in link instead" inside `SignInModal`) issues a 30-day server-only session for any email present in `account_recovery`. The cookie session is read on mount by `SignerContextProvider` and exposed as `emailSession` on `SignerContext` (alongside `refreshEmailSession`/`signOutEmailSession`); the existing `isLoggedIn` flag still requires a Nostr signer, so `emailSession` is purely additive — pages that consume it (currently `subscription-management.tsx` + `pages/subscriptions/manage.tsx`) treat it as authenticated, while order placement / messaging / profile screens still require a signer because they need to sign Nostr events. Subscription self-service flow (`/api/subscriptions/request-magic-link` → `/api/subscriptions/verify-magic-link`, page `/subscriptions/manage`, plus the "Email me a secure link" CTA on the guest section of subscription-management.tsx) issues a 15-minute session scoped to one `subscription_id`. `/api/stripe/{cancel,update}-subscription` accept either the existing Nostr signed-event proof OR the session cookie (subscription scope must match the target sub_id; email scope must match `buyer_email`). `/api/stripe/get-subscriptions` falls back to the session cookie when no `pubkey`/`email` query is given. The subscription-management UI omits the signed-event header and lets the server fall back to the cookie when no signer is available. Helpers in `utils/auth/session.ts` (token issue/consume, session create/lookup/delete, cookie helpers, `pruneMagicLinkArtifacts`); tests in `utils/auth/__tests__/session.test.ts`. Templates `signInMagicLinkEmail`/`subscriptionMagicLinkEmail`; rate-limited via `applyRateLimit`. Existence is never leaked on request endpoints. `/api/auth/signout` invalidates the DB row and clears the cookie; `/api/auth/session` returns the active session. Periodic cleanup runs in `pages/api/stripe/cron-cleanup.ts` (configurable `magic_link_max_age_days`, default 1). `subscriptions(LOWER(buyer_email))` expression index keeps the case-insensitive lookup cheap. Not exposed via MCP because consuming a magic link requires an email inbox; agents use the existing signed-event tools (`cancel_subscription`, etc.) instead.
- **Magic-Link Hardening**: (1) Per-email throttle: `countActiveTokensForEmail` caps unused live tokens at 3 per (email, scope) before issuing more. (2) Cookie rotation: `rotateSession` swaps the cookie token after every sensitive write (`/api/stripe/{cancel,update}-subscription`, `/api/auth/sessions DELETE`) so a leaked cookie has a short usable window. (3) CSRF: `SameSite=Strict` cookie + `isSameOriginRequest` Origin/Referer check on every cookie-authenticated POST/DELETE (returns 403 on mismatch). (4) `SignerContext` exposes `refreshEmailSession()` and is invoked after successful verify on `/auth/sign-in-link` and `/subscriptions/manage` so the cookie state lands in React without a reload. (5) Active-sessions UI: `GET /api/auth/sessions` lists live rows for the cookie's email, `DELETE` revokes all-other sessions; surfaced at `/auth/sessions` ("Sign out everywhere else"). (6) Timing-safe enumeration: `request-email-link` and `request-magic-link` enforce a constant-time floor (~600ms) regardless of whether the email exists, so response latency cannot be used to enumerate accounts/subscriptions. (7) Audit log: `magic_link_audit` table + `recordAuditEvent` (with `getRequestIp`/`getRequestUserAgent`) writes a row for every request/verify/cancel/update branch (success and failure) — fail-soft so audit errors never block the request. (8) `magic_link_max_age_days` is clamped to `[1, 30]` in `cron-cleanup.ts` so a misconfigured value cannot retain (or instantly purge) all sessions. (9) 401 mid-edit: `subscription-management.tsx` detects a 401 from the cookie-authenticated writes and shows a "Your secure email-link session has expired" banner with a "Request a new link" action instead of a generic failure. New helpers (`rotateSession`, `listActiveSessionsForEmail`, `deleteAllSessionsForEmail`, `countActiveTokensForEmail`, `recordAuditEvent`, `isSameOriginRequest`) all have Jest coverage in `utils/auth/__tests__/session.test.ts` (17 tests).
