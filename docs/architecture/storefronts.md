...
- **Seller flow** (`components/settings/custom-domain-section.tsx`, mounted inside `shop-profile-form.tsx`): submit domain → receive TXT (`_self-sown.<domain>`) + CNAME (subdomain) or A-record (apex, real IPs of `milk.market` resolved live + cached 5 min, overridable via `CUSTOM_DOMAIN_APEX_IPS`) instructions → "Check DNS" → status badge.
