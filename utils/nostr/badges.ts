...
const tags = (event: NostrEvent, key: string) =>
  event.tags
    .filter((value) => value[0] === key && value[1])
    .map((value) => value[1]!);
...
export function parseNip58BadgeAddress(address?: string) {
  ...
  const d = dParts.join(":");
  if (
    kind !== String(NIP58_BADGE_DEFINITION_KIND) ||
    !pubkey ||
    !isHexPubkey(pubkey) ||
    !d
  )
...
function parseBadgeSetAddress(address?: string) {
  const [kind, pubkey, ...dParts] = address?.split(":") || [];
  const d = dParts.join(":");
  return kind === String(NIP58_BADGE_SET_KIND) &&
    pubkey &&
    isHexPubkey(pubkey) &&
    d
    ? { pubkey, d, address: `${NIP58_BADGE_SET_KIND}:${pubkey}:${d}` }
    : null;
}
...
export function isNip58ProfileBadgesEvent(event: NostrEvent): boolean {
  return (
    ...
    (event.kind === NIP58_BADGE_SET_KIND &&
      tag(event.tags, "d") === NIP58_DEPRECATED_PROFILE_BADGES_D_TAG)
  );
}
...
export function parseNip58ProfileBadgesEvent(
  ...
  for (
    let i = 0;
    i < event.tags.length - 1 && parsed.length < MAX_NIP58_PROFILE_BADGES;
    i++
  ) {
    ...
    if (!seen.has(key)) {
      seen.add(key);
      parsed.push({
        definitionAddress: badge.address,
        awardEventId: e[1]!,
        definitionRelayHint: a?.[2],
        awardRelayHint: e[2],
      });
    }
    i++;
...
export function selectLatestNip58ProfileBadgesEvent(
  ...
  return events
    .filter(isNip58ProfileBadgesEvent)
...
function latestByAddress(events: NostrEvent[]) {
  ...
  for (const event of events) {
    const address = parseNip58BadgeAddress(
      `${event.kind}:${event.pubkey}:${tag(event.tags, "d") || ""}`
    );
    if (!address) continue;
    ...
    if (
      ...
      found.set(address.address, event);
  }
  return found;
}
...
export async function fetchNip58ProfileBadges(
  ...
  const profileResponse = await fetchEvents(
    nostr,
    [
  ...
  for (const event of profileResponse.events) {
    if (!users.includes(event.pubkey) || !isNip58ProfileBadgesEvent(event))
      continue;
  ...
  for (const user of users) {
    ...
    if (!event) {
      result.set(user, { badges: [], complete: profileResponse.complete });
      continue;
    }
    ...
    for (let i = 0; i < event.tags.length; i++) {
      const set =
        event.tags[i]?.[0] === "a"
          ? parseBadgeSetAddress(event.tags[i]?.[1])
          : null;
      if (set) {
        sets.push(set.address);
        setAddresses.add(set.address);
      }
    }
  ...
  for (const [user, sets] of selectedSets) {
    ...
    refs
      ...
      .forEach((ref) => awardIds.add(ref.awardEventId));
  }
  const configured = configuredRelays(relays);
  const awardsResponse = awardIds.size
    ? await fetchEvents(nostr, [{ kinds: [8], ids: [...awardIds] }], configured)
    : { events: [], complete: true };
  const awards = new Map(
    awardsResponse.events
      .filter((event) => event.kind === 8 && hexId.test(event.id))
  ...
  const definitionsResponse = definitionFilters.length
    ? await fetchEvents(nostr, definitionFilters, configured)
    : { events: [], complete: true };
  const definitions = latestByAddress(
    definitionsResponse.events.filter(
      (event) => event.kind === NIP58_BADGE_DEFINITION_KIND
    )
  );
  for (const [user, refs] of selected) {
    const badges: Nip58ProfileBadge[] = [];
    for (const ref of refs) {
      const event = definitions.get(ref.definitionAddress);
      const parsed = parseNip58BadgeAddress(ref.definitionAddress);
      if (event && parsed)
        badges.push({
          definitionAddress: parsed.address,
          awardEventId: ref.awardEventId,
          issuerPubkey: parsed.pubkey,
          badgeDefinitionDTag: parsed.d,
          name: tag(event.tags, "name") || parsed.d,
          description: tag(event.tags, "description"),
          image: tag(event.tags, "image"),
          thumbnail:
    ...
    result.set(user, {
      ...
      complete:
        ...
        setsResponse.complete &&
        awardsResponse.complete &&
        definitionsResponse.complete,
    });
  }
  return result;
}
...
function configuredRelays(relays: string[]): string[] {
  return [
    ...new Set(relays.map(hint).filter((value): value is string => !!value)),
  ];
}
