...
export const NWC_MIN_PASSPHRASE_LENGTH = 12;

type NWCEncryptionEnvelope = {
...
function normalizePassphrase(passphrase: string): string {
  ...
  if (normalized.length < NWC_MIN_PASSPHRASE_LENGTH) {
    throw new Error(
      `Passphrase must be at least ${NWC_MIN_PASSPHRASE_LENGTH} characters.`
    );
  }
  return normalized;
}
...
function isEnvelope(value: unknown): value is NWCEncryptionEnvelope {
  if (!value || typeof value !== "object") return false;
  const envelope = value as Partial<NWCEncryptionEnvelope>;
  return (
  ...
  const ciphertext = await crypto.subtle.encrypt(
    {
      name: "AES-GCM",
      iv,
      additionalData: textEncoder.encode(ENCRYPTION_CONTEXT),
      tagLength: AUTH_TAG_LENGTH,
    },
    key,
    textEncoder.encode(nwcString)
  );

  const envelope: NWCEncryptionEnvelope = {
...
export async function decryptNWCString(
  ...
  try {
    const parsed: unknown = JSON.parse(encryptedNWCString);
    ...
    const nwcString = textDecoder.decode(plaintext);
